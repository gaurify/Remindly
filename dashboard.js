...
function showFatalError(message) {
    ...
    els.errorBanner.classList.remove("hidden-init");
}

async function init() {
    if (!isConfigured) {
        showFatalError(
            "Remindly isn't connected to a backend yet. Add your Supabase project URL and anon key to js/config.js (see README.md)."
        );
        return;
    }
    ...
    try {
        ...
        console.error("Remindly dashboard failed to load:", err);
        showFatalError(
            `Something went wrong loading your dashboard: ${err.message || err}. ` +
                "If you just set up Supabase, make sure supabase/schema.sql has been run in the SQL editor."
        );
    }
}
...
async function loadProfile() {
    ...
    if (error) {
        ...
        if (upsertError) {
            throw new Error(
                `Couldn't load or create your profile (${upsertError.message}). Has supabase/schema.sql been run yet?`
            );
        }
        profile = defaults;
        return;
